...
$classes = Flux::classes()
    ->add('w-full border-b rounded-none block disabled:shadow-none dark:shadow-none')
    ...
    ->add(match ($variant) { // Text color
        'outline' => 'text-zinc-950 disabled:text-zinc-500 placeholder-zinc-400 disabled:placeholder-zinc-400/70 dark:text-zinc-300 dark:disabled:text-zinc-400 dark:placeholder-zinc-400 dark:disabled:placeholder-zinc-500',
    ...
    ->add(match ($variant) { // Border...
        'outline' => $invalid ? 'border-red-500' : 'shadow-none border-b-gray-800  disabled:border-b-zinc-200 dark:border-white/10 dark:disabled:border-white/5',
    ...
    <flux:with-field :$attributes :$name>
        <div {{ $attributes->only('class')->class('w-full relative block group/input') }} data-flux-input>
            <?php if (is_string($iconLeading)): ?>
                <div class="pointer-events-none absolute top-0 bottom-0 flex items-center justify-center text-xs text-gray-800 ps-0 start-0">
