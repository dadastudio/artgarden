...
$classes = Flux::classes()
    ->add(match ($variant) {
        default => 'p-10 [:where(&)]:max-w-[700px] shadow-none rounded-none',
...
<ui-modal {{ $attributes }} data-flux-modal>
    ...
    >
        ...
        <?php if ($closable): ?>
            <div class="absolute top-0 end-0 mt-1 me-1">
                <flux:modal.close>
                    <flux:button variant="ghost" icon="x-mark"  alt="Close modal" class="text-zinc-900! hover:text-green-700! dark:text-zinc-500! dark:hover:text-white!"></flux:button>
                </flux:modal.close>
            </div>
        <?php endif; ?>
    </dialog>
